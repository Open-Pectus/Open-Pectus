...
} from "../run-log-additional-values.component";
...
@Component({
  ...
  template: `
    ...
    >
      <div class="grid gap-2 px-3 py-2" [style.grid]="gridFormat()">
        ...
        <div class="col-end-6 flex gap-2">
          @if (runLogLine()?.forcible) {
            <app-run-log-line-force-button
              [lineId]="runLogLine()?.id"
              (click)="$event.stopPropagation()"
            />
          }
          @if (runLogLine()?.cancellable) {
            <app-run-log-line-cancel-button
              [lineId]="runLogLine()?.id"
              (click)="$event.stopPropagation()"
            />
          }
      ...
      <div
        [style.height.px]="
          expanded() && additionalValuesElementHasHeight
            ? additionalValues.scrollHeight
            : 0
        "
      ...
      >
        <div #additionalValues>
          @if (
            !runLogLine()?.start_values?.length &&
            !runLogLine()?.end_values?.length
          ) {
            <p class="text-end p-2">No additional values available.</p>
          }
          @if (runLogLine()?.start_values?.length) {
            <app-run-log-additional-values
              [type]="AdditionalValueType.Start"
              [values]="runLogLine()?.start_values"
            />
          }
          @if (runLogLine()?.end_values?.length) {
            <app-run-log-additional-values
              [type]="AdditionalValueType.End"
