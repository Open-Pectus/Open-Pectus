...
@Component({
  ...
  template: `
    <div class="flex justify-between" *ngrxLet="processUnit as processUnit">
      ...
      @if ((otherActiveUsers | ngrxPush)?.length !== 0) {
        <div class="flex flex-col items-end gap-1">
          ...
          <div class="grid grid-rows-5 items-end gap-y-1.5 gap-x-3 grid-flow-col" dir="rtl">
            @for (activeUser of (otherActiveUsers | ngrxPush); track activeUser.id) {
...
export class UnitHeaderComponent {
  ...
  protected otherActiveUsers = this.store.select(DetailsSelectors.otherActiveUsers);
