...
- [9. Unit Operation Definition](#9-unit-operation-definition)
  ...
  - [9.9 Common scenarios:](#99-common-scenarios)
...
Registers are defined using the `with_hardware_register(self, name: str, direction: RegisterDirection, **options):`
...
The UOD can define custom commands. These are python functions that define behavior for the unit. For example,
we might want a Reset command that performs a number of things, and make a simple 'Reset' command available in
pcode.
...
The actual implementation of the command, should be a function in the UOD, for example
```
...
TODO document how to use custom arguments in a command

## 9.7 Process values

In order for a `tag` to be shown by default in the OpenPectus web frontend, it must be defined as a process
value with `with_process_value`. This has three purposes:
- 1. Pick the tag to be displayed
- 2. Display the tag's current value (and unit if available)
- 3. Possibly allow the use to interact with the value, e.g. by setting a new value or excuting a command.

The following types of interaction is supported:
- Reading - Read-only. The tag value is displayed but it cannot be modified
- ReadingWithEntry - Read/write. The tag value is displayed and its value can be changed by clicking the
  value and typing the new desired value.
- ReadingWithChoice. The tag value is displayed and when clicked, a list of commands
  are shown. When a command from the list is selected, the corresponding command is executed.

TODO this API is not yet stable. It is intended to add methods such as `with_process_value_entry` and
`with_process_value_choice` instead of the current `with_process_value` that takes a Reading instance.

## 9.9 Common scenarios:
