    ...
    <div class="flex gap-3 items-center">
      <label class="relative">
        <input type="text" placeholder="Filter Run Log" size="20"
               ...
               #filterInput (input)="filterText.emit(filterInput.value)">
      ...
      <label class="flex items-center gap-1 cursor-pointer border rounded px-1 border-slate-200 h-8">
        In progress only
        <input type="checkbox" (input)="onlyRunning.emit(onlyRunningCheckbox.checked)" #onlyRunningCheckbox
...
export class RunLogFiltersComponent {
  ...
  @Output() onlyRunning = new EventEmitter<boolean>();
}
