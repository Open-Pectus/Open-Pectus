...
class SerializationTest(unittest.TestCase):
    ...
    def test_serialization_TagsUpdatedMsg(self):
        tu = EM.TagsUpdatedMsg(tags=[Mdl.TagValue(name="foo", timestamp=0, value="bar", value_unit="m")])
        tu_s = serialize(tu)
        self.assertIsNotNone(tu_s)

    def test_round_trip_TagsUpdatedMsg(self):
        tu = EM.TagsUpdatedMsg(tags=[Mdl.TagValue(name="foo", timestamp=0, value="bar", value_unit=None)])
        tu_s = serialize(tu)
        self.assertIsNotNone(tu_s)
