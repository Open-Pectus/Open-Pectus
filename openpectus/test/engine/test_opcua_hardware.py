...
class TestOPCUAHardware(unittest.TestCase):
    ...
    def test_can_write_multiple_registers(self):
        registers = [Register(f"{i:02d}", RegisterDirection.Both, path=f"Objects/2:writable_{i:02d}", safe_value=0.0) for i in range(100)]
        test_server = OPCUATestServer(registers=registers)
    ...
    def test_does_help_user_fix_path_typo(self):
        FT01_totalizer_1_total_with_typo = Register("FT01", RegisterDirection.Read, path="Objects/2:FT01/2:Totalizer/2:Totaal 1")
        FT01_totalizer_1_total_proper = Register("FT01", RegisterDirection.Read, path="Objects/2:FT01/2:Totalizer/2:Total 1")
    ...
    def test_can_read_write_registers_with_type(self):
        OPCUA_Types
        bool_register = Register("Bool", RegisterDirection.Both, path="Objects/2:Boolean value", type=OPCUA_Types.Boolean, safe_value=False)
        float_register = Register("Float", RegisterDirection.Both, path="Objects/2:Float value", type=OPCUA_Types.Float, safe_value=0.0)
        uint16_register =  Register("Uint16", RegisterDirection.Both, path="Objects/2:Uint16 value", type=OPCUA_Types.UInt16, safe_value=0)
        registers = [
            bool_register,
            float_register,
            uint16_register,
